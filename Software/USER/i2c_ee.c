...
#define I2C_PageSize           32			/* AT24C02每页有8个字节 */
...
void I2C_EE_BufferWrite(u8* pBuffer, u16 WriteAddr, u16 NumByteToWrite)
{
  ...
  Addr = (u8)(WriteAddr % I2C_PageSize);
...
void I2C_EE_ByteWrite(u8* pBuffer, u16 WriteAddr)
{
  ...
  I2C_SendData(I2C1, (u8)(WriteAddr/256));
 ...
 *         不能超过EEPROM页的大小。AT24C64每页有32个字节。
 * 输入  ：-pBuffer 缓冲区指针
 *         -WriteAddr 接收数据的EEPROM的地址 
 *         -NumByteToWrite 要写入EEPROM的字节数
 * 输出  ：无
...
{  
  ...
  /* Send the EEPROM's internal address to write to */   //H_ADDRESS
  I2C_SendData(I2C1, (u8)(ReadAddr/256));
